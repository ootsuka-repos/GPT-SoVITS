...
# os.environ['PYTORCH_ENABLE_MPS_FALLBACK'] = '1' # mpsがサポートしないステップに遭遇したらcpuを使用
import gradio as gr
...
def set_default():
    ...
    if version not in v3v4set:
        ...
        max_sovits_epoch = 16  # 40 # 3 # 訓練しすぎると危険
...
def fix_gpu_number(input):  # 範囲外のnumberを強制的に範囲内に変更
    try:
